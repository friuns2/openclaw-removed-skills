...
function getBaseUrl(api) {
    return api.config?.apiUrl || "https://pay.frontiercompute.io";
}
function getApiKey(api) {
    return api.config?.apiKey;
}
async function zap1Fetch(url, init) {
    const resp = await fetch(url, init);
    if (!resp.ok)
        throw new Error(`ZAP1 API returned ${resp.status}`);
    return resp.json();
}
export function createZap1Tools(api) {
    ...
    return [
        ...
        },
        {
            ...
            execute: async (_toolCallId, rawParams) => {
                ...
                if (!resp.ok)
                    throw new Error(`Decode returned ${resp.status}`);
        ...
        },
        {
            ...
            execute: async (_toolCallId, rawParams) => {
                ...
                if (!apiKey)
                    return jsonResult({ error: "API key required. Set apiKey in plugin config." });
                ...
                if (!resp.ok)
                    return jsonResult({ error: `${resp.status}: ${await resp.text()}` });
                return jsonResult(await resp.json());
            },
        },
        {
        ...
        },
        {
            ...
            execute: async (_toolCallId, rawParams) => {
                const agentId = readStringParam(rawParams, "agent_id", {}) || api.config?.agentId || "00zeven-alpha";
        ...
        },
        {
            ...
            execute: async (_toolCallId, _rawParams) => {
                const apiKey = getApiKey(api);
                if (!apiKey)
                    return jsonResult({ error: "API key required." });
