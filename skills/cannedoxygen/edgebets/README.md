...
You may see npm audit warnings about `bigint-buffer` - this is a known issue in the Solana ecosystem affecting all SPL Token packages. The vulnerability requires specific malicious input that doesn't occur in normal SDK usage. See [Solana SPL Token issues](https://github.com/solana-labs/solana-program-library/issues) for updates
