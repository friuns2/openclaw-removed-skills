...
# =============================================================================
# Linux 系统性能快照采集脚本
# performance-mastery skill - scripts/collect_snapshot.sh
# 用途：一键采集系统全维度性能数据，输出结构化报告
# 用法：bash collect_snapshot.sh [--output /path/to/report.txt]
# 注意：不使用 set -e，因为部分工具可能未安装，脚本需要跳过缺失工具继续采集其他数据
# =============================================================================
...
check_deps() {
    local missing=()
    local hints=()
    for cmd in iostat mpstat pidstat; do
        command -v "$cmd" &>/dev/null || { missing+=("$cmd"); hints+=("sysstat"); }
    done
    command -v iotop &>/dev/null || { missing+=("iotop"); hints+=("iotop"); }
    command -v smartctl &>/dev/null || { missing+=("smartctl"); hints+=("smartmontools"); }
    command -v numastat &>/dev/null || { missing+=("numastat"); hints+=("numactl"); }
    command -v slabtop &>/dev/null || { missing+=("slabtop"); hints+=("procps / util-linux"); }
    if [[ ${#missing[@]} -gt 0 ]]; then
        echo "⚠️  以下可选工具未安装（部分采集项将跳过）：" >&2
        local unique_hints=($(printf '%s\n' "${hints[@]}" | sort -u))
        for h in "${unique_hints[@]}"; do
            echo "   → 安装 $h: apt install $h / yum install $h" >&2
        done
        echo "" >&2
    fi
}
...
run_cmd "中断分布 TOP10" "cat /proc/interrupts | tail -n +2 | awk '{sum=0; for(i=2;i<=NF;i++){if(\$i~/^[0-9]+$/){sum+=\$i}} printf \"%12d %s\\n\", sum, \$0}' | sort -rn | head -10"
...
run_cmd "内存 cgroup 状态" "if [ -f /sys/fs/cgroup/memory.max ]; then echo 'cgroup v2:'; echo \"  limit: \$(cat /sys/fs/cgroup/memory.max)\"; echo \"  current: \$(cat /sys/fs/cgroup/memory.current)\"; cat /sys/fs/cgroup/memory.events 2>/dev/null | grep oom; elif [ -f /sys/fs/cgroup/memory/memory.limit_in_bytes ]; then echo 'cgroup v1:'; echo \"  usage: \$(cat /sys/fs/cgroup/memory/memory.usage_in_bytes)\"; echo \"  limit: \$(cat /sys/fs/cgroup/memory/memory.limit_in_bytes)\"; echo \"  failcnt: \$(cat /sys/fs/cgroup/memory/memory.failcnt 2>/dev/null)\"; else echo '无 cgroup 内存限制信息'; fi"
...
run_cmd "磁盘健康 (smartctl)" "for d in /dev/sd? /dev/vd? /dev/nvme?n?; do [ -b \"\$d\" ] && smartctl -H \$d 2>/dev/null | grep -E 'result|PASSED|FAILED' && echo \"\$d\"; done 2>/dev/null || echo 'smartctl 不可用'"
