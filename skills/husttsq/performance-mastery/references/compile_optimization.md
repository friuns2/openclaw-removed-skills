...
```bash
# GCC LTO 自动并行（推荐，速度与优化平衡）
gcc -O2 -flto=auto input1.c input2.c -o output
...
# 步骤2
gcc -O2 -fprofile-use -flto=auto -march=native \
    -fno-omit-frame-pointer \
    -o app_final src/*.c

# 场景4：Python 扩展模块（.so）
# 注：setup.py 方式已逐步过时，推荐使用 pyproject.toml + pip install
