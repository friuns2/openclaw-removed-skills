...
# 持久化
echo "vm.swappiness=10" >> /etc/sysctl.d/99-performance-mastery.conf
...
# 持久化
echo "vm.nr_hugepages=5120" >> /etc/sysctl.d/99-performance-mastery.conf
...
gcc -fsanitize=address -fno-omit-frame-pointer -g -O1 -o app_asan ./app.c
./app_asan   # 泄漏时会输出详细报告
