...
const SKILL_SLUG = 'next-video-gen';
...
function printBanner() {
  ...
  console.log(bold(cyan('║') + '   ' + bold('🎬  Next Video Gen Skill Installer') + '                    ' + bold(cyan('║'))));
  console.log(bold(cyan('║') + '       ' + dim('powered by Seedance 2.0 · Volcengine Ark') + '          ' + bold(cyan('║'))));
...
function printHelp() {
  ...
  console.log('  node bin/cli.js            ' + dim('# interactive installer'));
  console.log('  node bin/cli.js -y         ' + dim('# non-interactive (for AI agents / CI)'));
  console.log('  node bin/cli.js -y --path <dir>  ' + dim('# install to specific directory'));
  console.log('  node bin/cli.js --help     ' + dim('# show this help'));
  console.log('  node bin/cli.js --version  ' + dim('# show version'));
  console.log('');
  ...
  console.log('  1. Detects your agent skills directory');
  ...
  console.log('  DOUBAO_API_KEY   ' + dim('Your Volcengine Ark API key (get one at https://console.volcengine.com/ark)'));
  console.log('');
}
...
async function detectSkillsDir(rl, opts = {}) {
  ...
  const fallback = path.join(process.cwd(), 'skills');
...
async function setupApiKey(rl) {
  console.log(bold('\n[4/4] Volcengine Ark API key setup...'));

  const existing = process.env.DOUBAO_API_KEY;
  if (existing) {
    const masked = existing.slice(0, 6) + '••••••••••••••••';
    console.log(green('  ✓ DOUBAO_API_KEY is already set: ') + masked);
    return;
  }

  console.log(yellow('  ⚠  DOUBAO_API_KEY is not set.'));
  console.log('');
  console.log('  To generate videos you need a Volcengine Ark API key.');
  console.log(bold('  → Get one at: ') + cyan('https://console.volcengine.com/ark'));
  console.log('');
  ...
  if (!key) {
    ...
    console.log(dim('    export DOUBAO_API_KEY=your_key_here'));
    return;
  }
  ...
  const exportLine = shell.includes('fish')
    ? `set -x DOUBAO_API_KEY "${key}"`
    : `export DOUBAO_API_KEY="${key}"`;
  ...
  if (!addToRc.trim().toLowerCase().startsWith('n')) {
    try {
      fs.appendFileSync(rcFile, `\n# Volcengine Ark API key (added by next-video-gen installer)\n${exportLine}\n`);
...
function printSuccess(installPath) {
  ...
  console.log(bold(green('║') + '   ' + bold('✓  Next Video Gen skill installed successfully!') + '        ' + bold(green('║'))));
  console.log(bold(green('║') + '                                                          ' + bold(green('║'))));
  console.log(bold(green('╚══════════════════════════════════════════════════════════╝')));
  console.log('');
  ...
  console.log('  1. ' + dim('Ensure DOUBAO_API_KEY is set in your environment'));
  console.log('     ' + dim('export DOUBAO_API_KEY=your_key  (or add to .zshrc/.bashrc)'));
  console.log('  2. ' + dim('Load the skill in your agent and start generating videos'));
  console.log('     ' + dim('"Generate a 5-second 720p video of a sunset over the ocean"'));
  console.log('');
  console.log(dim('  Docs:    https://github.com/vennduan/next-video-gen'));
  console.log(dim('  API:     https://console.volcengine.com/ark'));
  console.log('');
}
  ...
  if (silent) {
    ...
    try {
      ...
      if (process.env.DOUBAO_API_KEY) {
        console.log(green('  ✓ DOUBAO_API_KEY is set.'));
      } else {
        console.log(yellow('  ⚠  DOUBAO_API_KEY is not set.'));
        console.log(dim('    Get one at: https://console.volcengine.com/ark'));
        console.log(dim('    Then run:   export DOUBAO_API_KEY=your_key'));
      }
