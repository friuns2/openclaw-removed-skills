            ...
            for n_digits in [4, 3, 2]:
                ...
                if garbled:
                    ...
                    if len(digits) >= 5:
                        ...
                        print(f"File: {p[:40]}: n_digits={n_digits}, garbled={garbled.group(1)!r} -> {val:.2f}")
                        break
            else:
                print(f"File: {p[:40]}: NO garbled match found")
                idx2 = segment.find('¥')
                if idx2 >= 0:
                    print(f"  ¥ context: {segment[idx2:idx2+40]!r}")
