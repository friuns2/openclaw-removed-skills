...
# === PharmaLab Dashboard Hook ===
sys.path.insert(0, os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', '..', 'pharmaclaw-lab-ui', 'scripts'))
try:
    from lab_hook import lab_update
except ImportError:
    def lab_update(*a, **kw): pass
...
def main():
    ...
    try:
        ...
        lab_update("pharmacology", "analyzing", f"ADME profiling {canonical_smiles[:40]}...",
        ...
        lab_update("pharmacology", "idle", "ADME profile complete")
        ...
        lab_update("pharmacology", "error", f"Failed: {str(e)[:60]}")
        output = {
            "agent": "pharma-pharmacology",
            "version": "1.1.0",
            "smiles": canonical_smiles,
