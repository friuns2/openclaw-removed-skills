...
sys.path.insert(0, os.path.join(script_dir, '..', '..', 'pharmaclaw-lab-ui', 'scripts'))
try:
...
def main():
    ...
    try:
        ...
        lab_update("chemistry_query", "executing",
                   f"Analyzing {canonical_smiles[:40]}...",
                   smiles=canonical_smiles,
                   compound=input_data.get("name"))
        ...
        confidence = 0.95
        risks = []
        lab_update("chemistry_query", "idle", "Analysis complete")
        ...
        risks = []
        viz_files = []
        lab_update("chemistry_query", "error", f"Failed: {str(e)[:60]}")
