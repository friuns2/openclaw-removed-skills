...
description: 基于 MCP 聚源金融数据库生成指定行业的 24 小时速报（默认 Markdown 格式，支持 HTML 输出）。覆盖备选简读素材、重要新闻信息、行业企业动态、投融资事件四大核心模块，所有信息可溯源、带原始发布时间戳。使用场景：当用户有以下意图时触发——生成指定行业的速报，快速了解某行业最新动态，如"生成新能源汽车行业速报"、"人工智能行业今天有什么新闻"、"帮我看看半导体行业的最新动态"、"医药行业 24 小时速报"等。Generate a 24-hour industry briefing based on the MCP GILData Financial Database (default in Markdown format, with HTML output supported). It shall cover four core modules: alternative brief reading materials, important news information, industry corporate developments, and investment and financing events. All information must be traceable and include the original publication timestamp. Use case: Triggered when the user intends to quickly get the latest updates on a specific industry.
metadata:
  openclaw:
    requires:
