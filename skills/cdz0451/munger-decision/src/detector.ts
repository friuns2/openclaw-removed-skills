...
export class ScenarioDetector {
  ...
  /** 场景缓存（ID -> Scenario） */
  private static scenarioCache: Map<string, Scenario> | null = null;
   ...
   */
  constructor() {
    if (!ScenarioDetector.scenarioCache) {
      const data = scenariosData as any;
      const rawScenarios = data.scenarios || scenariosData;
      ScenarioDetector.scenarioCache = new Map();
      rawScenarios.forEach((s: Scenario) => {
        ScenarioDetector.scenarioCache!.set(s.id, s);
      });
    }
    this.scenarios = Array.from(ScenarioDetector.scenarioCache.values());
  }
  ...
  getScenario(scenarioId: string): Scenario | undefined {
    return ScenarioDetector.scenarioCache!.get(scenarioId);
  }
