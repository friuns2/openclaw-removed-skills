...
export class ReportGenerator {
  ...
  formatMarkdown(report: DecisionReport): string {
    ...
    // 报告头部
    let md = `# 决策分析报告\n\n`;
    md += `**决策主题：** ${escapeMarkdown(report.title)}\n`;
    md += `**分析时间：** ${date}\n`;
    md += `**决策场景：** ${escapeMarkdown(report.scenario)}\n`;
    md += `**分析模型：** ${report.analyses.map(a => escapeMarkdown(a.modelName)).join('、')}\n\n`;
    md += `---\n\n`;

    // 各模型分析
    for (let i = 0; i < report.analyses.length; i++) {
      const analysis = report.analyses[i];
      md += `## ${i + 1}. ${escapeMarkdown(analysis.modelName)}分析\n\n`;
      md += `**评分：** ${escapeMarkdown(analysis.score)}\n`;
      md += `**分析：** ${escapeMarkdown(analysis.analysis)}\n\n`;
      
      // 如果有答案，展示答案列表
      if (analysis.answers.length > 0) {
        md += `**回答：**\n`;
        analysis.answers.forEach((answer, idx) => {
          md += `${idx + 1}. ${escapeMarkdown(answer)}\n`;
        });
        md += `\n`;
      }
    }
