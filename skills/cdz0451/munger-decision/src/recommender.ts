...
export class ModelRecommender {
  ...
  /** 模型缓存（ID -> Model） */
  private static modelCache: Map<string, Model> | null = null;
   ...
   */
  constructor() {
    if (!ModelRecommender.modelCache) {
      const rawModels = (modelsData as any).models || modelsData;
      ModelRecommender.modelCache = new Map();
      rawModels.forEach((m: Model) => {
        ModelRecommender.modelCache!.set(m.id.toLowerCase(), m);
      });
    }
    this.models = Array.from(ModelRecommender.modelCache.values());
  }
  ...
  recommend(modelIds: string[]): Model[] {
    return modelIds
      .map(id => ModelRecommender.modelCache!.get(id.toLowerCase().trim()))
  ...
  getModel(modelId: string): Model | undefined {
    return ModelRecommender.modelCache!.get(modelId.toLowerCase().trim());
  }
