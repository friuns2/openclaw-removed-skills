#!/usr/bin/env bash
# embed — Embed reference tool. Use when working with embed in devtools contexts.
...
show_help() {
    cat << 'HELPEOF'
embed v$VERSION — Embed Reference Tool
...
## What is Embed?
Embed (embed) is a specialized tool/concept in the devtools domain.
It provides essential capabilities for professionals working with embed.

## Key Concepts
- Core embed principles and fundamentals
- How embed fits into the broader devtools ecosystem  
- Essential terminology every practitioner should know

## Why Embed Matters
Understanding embed is critical for:
...
## Getting Started
1. Understand the basic embed concepts
...
## Installation
1. Download or clone the embed package
...
cmd_patterns() {
    cat << 'EOF'
# Embed — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for embed
...
cmd_performance() {
    cat << 'EOF'
# Embed — Performance Optimization
