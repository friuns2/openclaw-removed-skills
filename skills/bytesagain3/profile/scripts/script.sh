#!/usr/bin/env bash
# profile — Profile reference tool. Use when working with profile in devtools contexts.
...
show_help() {
    cat << 'HELPEOF'
profile v$VERSION — Profile Reference Tool
...
## What is Profile?
Profile (profile) is a specialized tool/concept in the devtools domain.
It provides essential capabilities for professionals working with profile.

## Key Concepts
- Core profile principles and fundamentals
- How profile fits into the broader devtools ecosystem  
- Essential terminology every practitioner should know

## Why Profile Matters
Understanding profile is critical for:
...
## Getting Started
1. Understand the basic profile concepts
...
## Installation
1. Download or clone the profile package
...
cmd_patterns() {
    cat << 'EOF'
# Profile — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for profile
...
cmd_migration() {
    cat << 'EOF'
# Profile — Migration & Upgrade Guide
